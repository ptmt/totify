...
// ищешь модель с name == 2ch, запоминаем её индекс, используем эту модель как начальную для фолдинга
...
// возвращаются набор предложений по контекстам - т.е. по доскам, добавляем их в индекс и делаем handleSentence, а затем сохраняем в index
