        ...
        let (min, max) =
             where 
                |> Seq.fold (fun (min, max) x -> 
                    ...
                    timer.Stop()
                    printfn "find_jaccard %f %f in %i ms" f s timer.ElapsedMilliseconds
                   // printfn "%A %A" s second
                    if f > s then (min + s, max + f) else (min + f, max + s)
                    ) (float 0, float 0)
        
        if max = float 0 then float 0 else min/max
...
let find_sim where = 
    ...
    matrix |> Seq.countBy (fun x-> x)
   
   
//    matrix 
//    |> Seq.take 100
// //   |> Array.ofSeq
//     |> Seq.map (fun elem -> (fst elem, snd elem, (sim_jaccard (fst elem) (snd elem) where)) ) 
